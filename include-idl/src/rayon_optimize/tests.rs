...
mod tests {
    ...
    fn test_nested_type_deduplication() {
        ...
        let mut optimizer = IdlOptimizer::new(config);
        let (processed, _metrics) = optimizer.optimize(test_idl).unwrap();
    ...
    fn test_compression_basic() {
        ...
        config.enable(OptimizationType::Compression)
            .set_compression_threshold(50)  // Keep the same threshold
            .set_compression_level(6);      // Then explicitly set level
        ...
        assert!(processed.get("compressed_data").is_some(), 
            "Data should be compressed with new thresholds");
    ...
    fn test_compression_threshold() {
        ...
        let mut config = OptimizationConfig::default();
        config.enable(OptimizationType::Compression)
            .set_compression_threshold(1000); 
    ...
    fn test_nested_compression() {
        ...
        let mut optimizer = IdlOptimizer::new(config);
        let (_processed, metrics) = optimizer.optimize(test_idl).unwrap();
